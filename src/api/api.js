export async function fetchlatlon(){
    ...
    try {
      ...
      const response = await fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${ document.getElementById("location").value}&limit=1&appid=${APIkey}`);
  ...
  export async function fetchData(){
    ...
    try {
      ...
      const jsonData = await response.json();
      document.getElementById('curimg').src=`http://openweathermap.org/img/wn/${jsonData.current.weather[0].icon}@4x.png`;
